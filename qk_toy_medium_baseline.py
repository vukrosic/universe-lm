#!/usr/bin/env python3
"""Run ToyMediumConfig with gain=0.0 (baseline)."""

import sys, json, time
sys.path.insert(0, "/workspace/universe-lm-qk-pilot")

from configs.llm_config import ToyMediumConfig
...
print(f"\n=== qk_gain={GAIN} (baseline) ===")
print(f"  final val_loss: {r['final_vl']:.4f}")
print(f"  wall time: {r['wall']:.1f}s")

with open("/tmp/toy_medium_baseline.json", "w") as f:
    json.dump(r, f, indent=2)
print("Saved to /tmp/toy_medium_baseline.json")