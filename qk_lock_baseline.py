#!/usr/bin/env python3
"""Run Lock10M200MConfig baseline (gain=0.0)."""

import sys, json, time
sys.path.insert(0, "/workspace/universe-lm-qk-pilot")

from configs.llm_config import Lock10M200MConfig
...
print(f"\n=== Lock10M200MConfig gain={GAIN} (baseline) ===")
print(f"  final val_loss: {r['final_vl']:.4f}")
print(f"  wall time: {r['wall']:.1f}s ({r['wall']/60:.1f}min)")

with open("/tmp/lock10m200m_baseline.json", "w") as f:
    json.dump(r, f, indent=2)
print("Saved to /tmp/lock10m200m_baseline.json")