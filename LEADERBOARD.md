...
| # | Val loss | Δ vs ctrl | Run | Summary | Date | Evidence |
|---|---|---|---|---|---|---|
| 18 | **4.6676** | **-0.1308** | vq-gain+swa | V+q_gain + sliding-window attention (window=512) (`#51`) at natural end, 4,883 steps. **Multi-seed mean = 4.6676, std = 0.0034** across seeds 42/43 (4.6700, 4.6652) — exceptionally reproducible, **tighter than V+q_gain**. **NEW BEST screen20m record** (was V+q_gain 3-seed mean 4.6815). -0.014 below V+q_gain 3-seed mean (~2.4σ). Flag-only, no extra params — SDPA's `is_causal=True` is replaced by an explicit causal-local boolean mask (density 0.2188 at window=512, seq=2048). **The attention pattern was a hidden lever** — window 512 = quarter of seq_len 2048 captures the useful long-range signal and the rest was noise. | 2026-06-03 | `runs/s_vqgain_swa_full/metrics.json` · `runs/s_vqgain_swa_s43/metrics.json` |
