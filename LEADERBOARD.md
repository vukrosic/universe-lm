...
| # | Val loss | Δ vs ctrl | Run | Summary | Date | Evidence |
|---|---|---|---|---|---|---|
| 0 | **4.6745** | -0.1239 | voq-embeddings | V+O + per-head Q-gain (`#38`) at natural end, 4,883 steps. **New screen20m #0.** V (inside attention) + O (post-O residual) + per-head Q-gain. **Multi-seed mean = 4.6789, std = 0.0048 across seeds 42/43/44** (4.6745, 4.6766, 4.6855) — exceptionally reproducible. Beats V+O (4.7188) by 0.0443, beats control (4.7984) by 0.1239. The q_gain lever is a per-head learnable scalar on Q (init zero, multiplies Q after RoPE), cost = 144 extra params (negligible). This is the first non-embed lever to clearly help. | 2026-06-02 | `runs/s_voqgain_full/metrics.json` · `runs/s_voqgain_s43/` · `runs/s_voqgain_s44/` |
| 1 | 4.7188 | -0.0796 | vo-embeddings | V+O combo (`#35`) at natural end, 4,883 steps. V (inside attention) + O (post-O residual) — additive. Beats V+Q (4.7428) by 0.0240, beats V (4.7728) by 0.0540, beats control by 0.0796. | 2026-06-02 | `runs/s_voembed_full/metrics.json` · `logs/s_voembed_full.log` |
