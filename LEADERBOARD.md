...
| # | Val loss | Δ vs ctrl | Run | Summary | Date | Evidence |
|---|---|---|---|---|---|---|
| 0 | **4.6745** | -0.1239 | voq-embeddings | V+O + per-head Q-gain (`#38`) at natural end, 4,883 steps. **New screen20m #0.** V (inside attention) + O (post-O residual) + per-head Q-gain. Beats V+O (4.7188) by **0.0443** — q_gain is **strongly additive** with the best embed combo. Beats control (4.7984) by **0.1239**. The q_gain lever is a per-head learnable scalar on Q (init zero, multiplies Q after RoPE), cost = 144 extra params (negligible). This is the first non-embed lever to clearly help. | 2026-06-02 | `runs/s_voqgain_full/metrics.json` · `logs/s_voqgain_full.log` |
