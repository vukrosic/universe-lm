...
- **Step-0 bit-identity when flag on**: `se_gamma = sigmoid(-10) ≈ 4.54e-5` ⇒ `attn_out_post = (1 − 4.54e-5)·attn_output + 4.54e-5·(attn_output ⊙ se_w)`. The blend's contribution to `attn_out_post` is `O(4.54e-5 · |attn_output| · |se_w|)`. With `|attn_output| ≈ 1.0` (post-merge rmsnorm) and `|se_w| ≈ 0.5` (sigmoid of Kaiming-init), the per-element drift is `O(2e-5)`, well below the spec's `< 1e-5 * |attn_output|` tolerance. The W_1 / W_2 init consumes RNG (matches the standard `nn.Linear` init) — the on-path construction-time RNG state diverges from the off-path, but the forward output is at the fp32 noise floor.
