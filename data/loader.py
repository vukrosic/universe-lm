...
def finalize_dataset(dataset: Union[Dataset, IterableDataset], config: DataConfig) -> Dataset:
    ...
    # Set PyTorch format - only use columns that exist
    # Note: attention_mask is generated by tokenizer but not needed for causal LM
    available_columns = ["input_ids", "labels"]
    if "attention_mask" in dataset.column_names:
        available_columns.append("attention_mask")
    
    dataset.set_format(type="torch", columns=available_columns)
